...
import {
  UpdateCategoryBySupplyCategoryDto,
  UpdateSupplyCategoryDto,
} from './dto/update-supply.dto';
...
export class SupplyCategoryService {
  ...
  async getSupplyCategories() {
    try {
      const supplyCategory: SupplyCategory[] =
        await this._supplyCategoryRepository.find();
  ...
  async getCategory() {
    try {
      const supplyCategoryQuery: SupplyCategory[] =
        await this._supplyCategoryRepository.find({
          relations: ['supplyType', 'supplyCategory'],
          where: {
            is_active: true,
          },
        });
      ...
      const supplyCategory: SupplyCategory[] = supplyCategoryQuery.map(
        (supply) => ({
          ...supply,
          supplyCategory: supply.supplyCategory.filter(
  ...
  async createSupplyCategory(
    user: TokenDto,
    createSupplyDto: CreateSupplyCategoryDto,
  ) {
    try {
      ...
      const nameExist: SupplyCategory[] =
        await this._supplyCategoryRepository.find({
          where: {
            supply_name,
            is_active: true,
      ...
      const supplyCategory: SupplyCategory =
        await this._supplyCategoryRepository.save({
          supply_name,
          supply_type_id,
    ...
    try {
      ...
      if (updateSupplyDto.is_active === true) {
        ...
        const existingCategories =
          await this._supplyCategoryBySupplyRepository.findBy({
            supply_id: id,
            is_active: true,
          });

        for (const existingCategory of existingCategories) {
          const isCategoryProvided = categoryBySupply.some(
            (category) =>
              category.supply_category_name ===
              existingCategory.supply_category_name,
          );
  ...
  async createCategoryBySupply(
    user: TokenDto,
    createCategoryBySupplyDto: CreateCategoryBySupplyDto,
  ) {
    try {
      const { supply_id, supply_category_name, quantity } =
        createCategoryBySupplyDto;
      ...
      const categoryBySupplyExist: CategoryBySupply[] =
        await this._supplyCategoryBySupplyRepository.find({
          where: {
      ...
      const newCategoryBySupply: CategoryBySupply =
        await this._supplyCategoryBySupplyRepository.save({
          supply_category_name,
          supply_id,
          quantity,
      ...
      return {
        ...
        message: `La categoria del insumo ${supply_category_name} ha sido creada satisfactoriamente`,
        status: HttpStatus.CREATED,
      };
    } catch (error) {
      return this._handlerError.returnErrorRes({ error, debug: true });
  ...
  async deleteSupplyCategory(id: number, user: TokenDto) {
    try {
      const supplyCategoryExist: SupplyCategory =
